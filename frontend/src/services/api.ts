...
export const batchAPI = {
  uploadExams: (file: File) => {
    ...
    return api.post<{ jobId: string; status: string }>("/batch/exams", formData);
  },
  uploadScripts: (file: File, examId: string) => {
    const formData = new FormData();
    formData.append("file", file);
    formData.append("examId", examId);
    return api.post<{ jobId: string; status: string }>("/batch/scripts", formData);
  },
  listJobs: () => api.get<{ items: any[] }>("/batch/jobs"),
  getNotifications: () =>
    api.get('/notifications').then(res => res.data),
  markNotificationsRead: () =>
    api.post('/notifications/mark-read').then(res => res.data),
  deleteJob: (jobId: string) => api.delete(`/batch/jobs/${jobId}`),
  getJob: (jobId: string) => api.get<any>(`/batch/jobs/${jobId}`),
};
