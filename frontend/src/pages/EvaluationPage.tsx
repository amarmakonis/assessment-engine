...
export function EvaluationPage() {
  ...
  function loadData() {
    ...
    evaluationAPI
      ...
      .then(({ data: d }) => {
        setData(d);
        const first = Array.isArray(d.evaluations) ? d.evaluations[0] : null;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        <div className="text-right">
          ...
          <p className="text-xs text-text-muted mt-0.5">
            Full paper{data.examTotalMarks ? ` • Exam total ${data.examTotalMarks}` : ""}
          </p>
        </div>
      </div>
      ...
      <div className="space-y-8">
        {(() => {
          // Group evaluations by context
          const groups: { context?: string; evals: EvaluationResult[] }[] = [];
          data.evaluations.forEach((ev) => {
            const questionInfo = data.questions?.find((q) => q.questionId === ev.questionId);
            const context = questionInfo?.context;
            
            const lastGroup = groups[groups.length - 1];
            if (lastGroup && lastGroup.context === context) {
              lastGroup.evals.push(ev);
            } else {
              groups.push({ context, evals: [ev] });
            }
          });
          ...
          return groups.map((group, gIdx) => (
            <div key={gIdx} className="space-y-4">
              {group.context && (
                <div className="rounded-2xl border border-violet-200 bg-violet-50/40 p-6 shadow-sm mb-6">
                  <div className="flex items-center gap-2 mb-3">
                    <div className="w-8 h-8 rounded-lg bg-violet-100 flex items-center justify-center">
                      <BookOpen className="w-4 h-4 text-accent-purple" />
                    </div>
                    <h3 className="text-sm font-bold text-accent-purple uppercase tracking-widest">Reading Passage / Context</h3>
                  </div>
                  <p className="text-text-primary leading-relaxed whitespace-pre-wrap">{group.context}</p>
                </div>
              )}
                ...
                {group.evals.map((ev) => {
                  ...
                  return (
                    <QuestionAccordion
                      ...
                      // We don't pass contextText here because it's shown in the group header
                      ...
                      onDelete={() => setDeleteResultId(`${scriptId}:${ev.questionId}`)}
...
function QuestionAccordion({
  ...
  const criterionScores = Array.isArray(ev.criterionScores) ? ev.criterionScores : [];
  const feedback =
    ev.feedback && typeof ev.feedback === "object"
      ? ev.feedback
      : ev.feedback
        ? { summary: String(ev.feedback), strengths: [], improvements: [], encouragementNote: "" }
        : null;
        ...
        <div className="border-t border-border p-5 space-y-6 animate-in">
            ...
            {questionText && (
              <div className="rounded-xl border border-border bg-blue-50/50 p-4">
                ...
                <div className="flex gap-2">
                   <span className="font-bold text-accent-blue shrink-0">{questionLabel}</span>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div className="space-y-3">
                {criterionScores.length === 0 ? (
                  <div className="bg-card-alt rounded-lg p-3 border border-border">
                    <p className="text-sm text-text-secondary">
                      Detailed criterion breakdown is not available for this evaluation result.
                    </p>
                  </div>
            ...
            <div>
              ...
              {criterionScores.length > 0 ? (
                <ScoreBreakdownChart scores={criterionScores} />
              ) : (
                <div className="bg-card-alt border border-border rounded-lg p-4 text-sm text-text-secondary">
                  No score distribution available for this evaluation result.
                </div>
              )}
            </div>
          </div>
          ...
          {feedback && (
            <div>
              ...
              <div className="bg-card-alt border border-border rounded-lg p-4 space-y-4">
                <p className="text-sm text-text-primary leading-relaxed">{feedback.summary}</p>
                ...
                {feedback.improvements?.length > 0 && (
                  <div className="bg-amber-50 border border-amber-200 rounded-lg p-3">
                    ...
                    <div className="space-y-2">
                      {feedback.improvements.map((imp: any) => (
